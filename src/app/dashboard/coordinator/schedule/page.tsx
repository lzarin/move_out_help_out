...
export default async function CoordinatorSchedulePage() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-semibold text-stone-900">All pickup windows</h2>
      {windows.length === 0 ? (
          ...
          {windows.map((w) => (
            <li key={w.id} className="card">
              <div className="flex items-start gap-3">
                <div className="rounded-lg bg-warm-100 p-2 text-warm-600">
                ...
                <div>
                  ...
                  {w.donor.address && (
                    <p className="mt-1 text-xs text-stone-500">{w.donor.address}</p>
                  )}
                  {w.assignment ? (
                    <p className="mt-2 flex items-center gap-1 text-sm text-brand-700">
                      <Truck className="h-4 w-4" />
                      {w.assignment.nonprofit.name}
                    </p>
                  ) : (
                    <p className="mt-2 text-sm text-stone-500">Unassigned</p>
                  )}
